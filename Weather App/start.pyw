...
def find_weather(temp):
    program_current_status['text'] = 'Getting Weather...'
    if search_bar.get() == None:
        ...
        try:
            ...
            button['state'] = NORMAL
            program_current_status['text'] = 'Failed!'
...
program_current_status = Label(root, font=('Ariel', 10), bg='light green')
program_current_status.place(x=2, y=2)

button = Button(text='GO', foreground='#1f1f1f', background='Pink', font=('Times', 20),
                command=lambda: starter(0), borderwidth=0, activebackground='#bf9199', width=10)
